...
rawdata = "dataset/crop_production.csv"
...
print('Data Set Shape: {0}',df.State_Name.shape)
print('Data set columns are {0}',df.columns)
...
print('After removing na rows, Data Set Shape: {0}',df.State_Name.shape)
...
def training_testing_phase(regression_model, name):
    ...
    print("Fit a " + name + " model X_train, and calculate MAE with y_train:",
          metrics.mean_absolute_error(pred_train, y_train))
    print("Fit a " + name + " X_train, and calculate MAE with X_test, y_test:",
